...
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	 ...
	 *                If the text for the current editing component has already
	 *                been set.
	 */
	 ...
	 *                If the specified enumeration value does not represent a
	 *                color.
	 */
	...
	public FancyMessage statisticTooltip(final Statistic which, final Material item) {
		...
		try {
			final Object statistic = Reflection.getMethod(Reflection.getOBCClass("CraftStatistic"), "getMaterialStatistic", Statistic.class, Material.class).invoke(null,
			        which, item);
			return achievementTooltip((String) Reflection.getField(Reflection.getNMSClass("Statistic"), "name").get(statistic));
		} catch (final IllegalAccessException e) {
			Bukkit.getLogger().log(Level.WARNING, "Could not access method.", e);
	...
	public FancyMessage statisticTooltip(final Statistic which, final EntityType entity) {
		...
		try {
			final Object statistic = Reflection.getMethod(Reflection.getOBCClass("CraftStatistic"), "getEntityStatistic", Statistic.class, EntityType.class).invoke(null,
			        which, entity);
			return achievementTooltip((String) Reflection.getField(Reflection.getNMSClass("Statistic"), "name").get(statistic));
		} catch (final IllegalAccessException e) {
			Bukkit.getLogger().log(Level.WARNING, "Could not access method.", e);
	...
	private Object createChatPacket(final String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException {
		...
		final Object serializedChatComponent = nmsChatSerializerGsonInstance.fromJson(json, Reflection.getNMSClass("IChatBaseComponent"));
		...
		for (final JsonElement mPrt : extra) {
			...
			for (final Map.Entry<String, JsonElement> entry : messagePart.entrySet()) {
				...
				if (TextualComponent.isTextKey(entry.getKey())) {
					...
					final Map<String, Object> serializedMapForm = new HashMap<String, Object>(); // Must
					// be
					// object
					// due
					// to
