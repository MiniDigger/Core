...
public abstract class TextualComponent implements Cloneable {
	...
	static TextualComponent deserialize(final Map<String, Object> map) {
		...
		} else if (map.size() >= 2 && map.containsKey("key") && !map.containsKey(
		        "value") /* It contains keys that START WITH value */) {
			// Complex JSON object
		...
		public static ComplexTextTypeComponent deserialize(final Map<String, Object> map) {
			...
			for (final Map.Entry<String, Object> valEntry : map.entrySet()) {
				if (valEntry.getKey().equals("key")) {
					...
					value.put(valEntry.getKey()
					        .substring(6) /* Strips out the value prefix */, valEntry.getValue().toString());
				}
			}
