...
public class V3Generator {
	 ...
	 * @return
	 */
	public static List<V3CameraFrame> line(final Vector pos1, final Vector pos2, final int fps, final int seconds, final float yaw, final float pitch) {
		final List<V3CameraFrame> frameList = new ArrayList<V3CameraFrame>();
		...
		for (int frameNum = 0; frameNum < frameCount; frameNum++) {
			...
			// Pridaj ramec.
			frameList.add(new V3CameraFrame(pos1.getX(), pos1.getY(), pos1.getZ(), yaw, pitch, false));
		}
		return frameList;
	}
	
	public static List<V3CameraFrame> lineLookAt(final Vector pos1, final Vector pos2, final Vector lookAt, final int fps, final int seconds) {
		final List<V3CameraFrame> frameList = new ArrayList<V3CameraFrame>();
		final int frameCount = fps * seconds;
		// Vypocitaj vzdialenosti.
		final Vector positionDifference = pos2.subtract(pos1);
	 ...
	 * @return
	 */
	public static List<V3CameraFrame> flyInCirleLookAt(final Vector center, final Vector lookAt, final int radius, final int fps, final int seconds, final float speed) {
		final List<V3CameraFrame> frameList = new ArrayList<V3CameraFrame>();
		final int frameCount = fps * seconds;
		for (int frameNum = 0; frameNum < frameCount; frameNum++) {
