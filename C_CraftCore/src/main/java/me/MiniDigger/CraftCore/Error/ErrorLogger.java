...
public class ErrorLogger extends PluginLogger {
	...
	public void log(LogRecord logRecord) {
		if (!generateErrorLog(logRecord)) super.log(logRecord);
	}
	...
	private static boolean generateErrorLog(LogRecord record) {
		Throwable thorn;
		if ((thorn = record.getThrown()) == null) return false;
		...
		if (problematicPlugin == null) return true; // Plugin doesn't exist!
		...
		try {
			...
			try {
				File dump = new File(new File(problematicPlugin.getDataFolder(), "errors").getAbsoluteFile(), String.format("%s_%s.error.log", thorn.getClass()
				        .getSimpleName(), // Get first 6 chars of hash
