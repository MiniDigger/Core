	...
	public boolean checkCombability() {
		...
		for (final Feature f : features) {
			for (final FeatureType s : f.getDependencies()) {
				if (!fType.contains(s)) {
					Core.getCore().getInstance().info("Dependency " + s + " for Feature " + f.getType() + " in Game " + getGame().getIdentifier().toString() + " is missing!");
					return false;
				}
			}
			...
			for (final FeatureType s : f.getIncompabilities()) {
				if (fType.contains(s)) {
					Core.getCore().getInstance().error("Dependency " + s + " is incompatible with Feature " + f.getType() + " in Game " + getGame().getIdentifier().toString() + "!");
					return false;
				}
			}
		}
