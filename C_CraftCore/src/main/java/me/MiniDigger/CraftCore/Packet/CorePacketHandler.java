...
public class CorePacketHandler implements PacketHandler {
	...
	public void sendPacket(Packet packet, Session session) {
		for (WebSocket socket : ((WebSocketServer) Core.getCore().getSocketHandler().getServer()).connections()) {
	...
	public void handleIncome(String msg, WebSocket con) {
		...
		try {
			Packet pa = packet.getConstructor().newInstance();
			pa.fromString(msg);
			Session s = Core.getCore().getSocketHandler().getSession(con.getRemoteSocketAddress());
