...
public class CoreCommonMethods implements CommonMethods {
	...
	public void printJoinMessage(final User user) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(Core.getCore().getInstance(), new Runnable() {
			...
			public void run() {
				try {
					final ImageMessage msg = new ImageMessage(ImageIO.read(new URL("https://minotar.net/avatar/" + user.getRealName() + "/200.png")), 8, ImageChar.DARK_SHADE.getChar());
		...
		new BukkitRunnable() {
			...
			public void run() {
				try {
					...
					// > 5) {
					// Prefix.API.getPrefix().then("Es sieht so aus als würdest
					// du 1.8.* oder einen Snapshot
					// benutzen.").color(ChatColor.GOLD).style(ChatColor.BOLD)
					// .send(user.getPlayer());
					// Prefix.API.getPrefix().then("Dies kannst du tun, kann
					// aber zu Problemen
					// führen").color(ChatColor.GOLD).style(ChatColor.BOLD)
					// .send(user.getPlayer());
					// }
	...
	public void killPlugin() {
		// Core.getCore().getInstance().debug("Licene check failed but I don't
		// care as long as you registered it at startup");
		Bukkit.getConsoleSender().sendMessage("[Core] " + ChatColor.RED + "[Error] The Plugin decided to kill itselve");
		Bukkit.getConsoleSender().sendMessage("[Core] " + ChatColor.RED + "[Error] This mostly happens, if your licence is wrong, or a other server uses the same licence");
