...
public class SquadCommands {
	...
	public void join(final CommandArgs args) {
		...
		if (invs.size() == 0) {
			...
			if (s.join(args.getUser().getUUID())) {
				args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Du bist dem Squad von ").color(ChatColor.GREEN).then(Core.getCore().getUserHandler().get(s.getOwner()).getDisplayName()).color(ChatColor.YELLOW)
						.then(" beigetreten!").color(ChatColor.GREEN));
			} else {
				args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Du konntest dem Squad nicht betreten!").color(ChatColor.RED));
				args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Entweder das Squad ist voll, du bist schon in dem Squad oder die Einladung wurde zurückgezogen!").color(ChatColor.RED));
			}
			return;
		}
		...
		for (final Squad squad : invs) {
			args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then(Core.getCore().getUserHandler().get(squad.getOwner()).getDisplayName()).color(one)
					.command("/squad join " + Core.getCore().getUserHandler().get(squad.getOwner()).getDisplayName()).then(": " + squad.getMembers().size() + " Mitglieder").color(two));
		}
	...
	public void leave(final CommandArgs args) {
		...
		if (s.getOwner().equals(args.getUser().getUUID())) {
			...
			args.getUser().sendMessage(Prefix.HINT.getPrefix().then("Benutze ").color(ChatColor.GOLD).then("squad delete ").color(ChatColor.YELLOW).then("um das Squad aufzulösen").color(ChatColor.GOLD));
			return;
		}
	...
	public void invite(final CommandArgs args) {
		...
		if (p == null) {
			args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Spieler ").color(ChatColor.RED).then(args.getArgs()[0]).color(ChatColor.YELLOW).then(" konnte nicht gefunden werden!").color(ChatColor.RED));
			return;
		}
		...
		args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Du hast ").color(ChatColor.GREEN).then(user.getDisplayName()).color(ChatColor.YELLOW).then(" in dein Squad eingeladen!").color(ChatColor.GREEN));
	}
	...
	public void kick(final CommandArgs args) {
		...
		args.getUser().sendMessage(Prefix.SQUAD.getPrefix().then("Du hast ").color(ChatColor.GREEN).then(user.getDisplayName()).color(ChatColor.YELLOW).then(" aus deinem Squad gekickt!").color(ChatColor.GREEN));
	}
}
