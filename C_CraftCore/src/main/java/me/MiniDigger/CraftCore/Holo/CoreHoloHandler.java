...
public class CoreHoloHandler implements HoloHandler {
	...
	private Color[]	colors	  = { Color.fromRGB(0, 0, 0), Color.fromRGB(0, 0, 170), Color.fromRGB(0, 170, 0), Color.fromRGB(0, 170, 170), Color.fromRGB(170, 0, 0),
	...
	public List<HoloList> loadImage(String url) {
		...
		if (url.endsWith(".gif")) {
			try {
				...
				List<HoloList> tags = new ArrayList<>();
				for (int i = 0; i < frames; i++) {
					
					lines = getLinesFromImage(reader.read(i));
					
					HoloList nametagList = new CoreHoloList();
	...
	class UpdateNametagTask extends BukkitRunnable {
		...
		private List<HoloList>	tags;
		
		public UpdateNametagTask(int delay, List<HoloList> tags) {
			this.delay = delay;
			this.tags = tags;
		}
		...
		public void run() {
			for (int i = 0; i < tags.get(frame).getHolos().size(); i++) {
				Holo nametag = tags.get(frame).getHolos().get(i);
				Holo nametag2 = tags.get(next).getHolos().get(i);
