...
public class MegaHAL {
	 ...
	 */
	public MegaHAL(File data) throws IOException {
		/*
		...
		Set<Symbol> banWords = Utils.readSymbolSetFromFile(data.getAbsolutePath() + "/megahal.ban");
		Set<Symbol> auxWords = Utils.readSymbolSetFromFile(data.getAbsolutePath() + "/megahal.aux");
		...
		BufferedReader reader = new BufferedReader(new FileReader(data.getAbsolutePath() + "/megahal.trn"));
		String line;
		int trainCount = 0;
