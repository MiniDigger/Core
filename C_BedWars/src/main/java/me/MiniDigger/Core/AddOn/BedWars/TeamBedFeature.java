...
public class TeamBedFeature extends CoreFeature {
	...
	public void start() {
		...
		for (int i = 0; i < numTeams; i++) {
			final BedFeature f = new BedFeature(getPhase(), ((TeamSpawnFeature) getPhase().getFeature(FeatureType.TEAM_SPAWN)).getSpawn(teams.get(i)),
			        teams.get(i).getName());
			f.init(getPhase());
			f.start();
